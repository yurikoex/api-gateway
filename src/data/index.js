...
export const getDataInstance = ({ logger }) =>
	new Promise((resolve, reject) => {
		logger.next(`connecting to redis`)
		const client = createClient(process.env.DOCKER ? { host: `redis` } : void 0)
			...
			new Promise((resolve, reject) => {
				...
				client.hgetall('services', (err, replies) => {
					if (err) {
						...
						replies !== null ? resolve(_.map(replies, (config, route) => ({ route, config: JSON.parse(config) }))) : resolve([])
					}
				})
			})

		const registerService = ({ route, config }) =>
			new Promise((resolve, reject) => {
